...
void delete_node(SLL **ptr ,int n)
{
	SLL *del,*prev;
	int c1;
	c1=count(*ptr);
	...
	if(n>=1 && n<=c1)
	{
		del=*ptr; //holding staring node address
		for(int i=1;i<=c1;i++)
		{
			if(i==n)
			{
				if(i==1)
					*ptr=del->next;// if first node we deleted the headptr should be update
				else
					prev->next=del->next;//update prev node next
				free(del);
				break;
			}
			prev=del;// holding deleting node,prev node address
			del=del->next;//update next node 
